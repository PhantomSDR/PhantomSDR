...
/*
template <typename T, size_t decimation = 4> class MovingAverage {
  public:
    MovingAverage(size_t length) : length{length}, idx{0} {
        while (length) {
            q.push_back(std::vector<T>(length, 0));
            if (length == 1) break;
            length = (length + decimation - 1) / decimation;
        }
    }
    MovingAverage() : MovingAverage(4) {}
    static T add4(T *arr) { return arr[0] + arr[1] + arr[2] + arr[3]; }
    inline T insert(T val) {
        size_t q_len = q.size();
        q[0][idx] = val;
        for (size_t i = 1; i < q_len; i++) {
            idx /= 4;
            q[i][idx] = add4(&q[i - 1][idx * 4]);
        }
        idx = (idx + 1) % length;
    }
    inline T getAverage() { return q.back()[0]; }
    inline T* getBuffer() { return q.back().data(); }
  protected:
    size_t length;
    size_t idx;
